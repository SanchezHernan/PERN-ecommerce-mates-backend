...
app.get('/carrito/productos/:cartId', async(req, res) => {
    try {
        ...
        const productos = await pool.query(
            'select p.nombre, p.precio, p.porcdescuento, l.cantidadproducto, l.codigo, 0 as opc from producto p, linea l where p.codigo = l.producto and l.carrito = $1 union select c.nombre, c.precio, 0 as porcdescuento, l.cantidadproducto, l.codigo, 1 as opc from combo c, linea l where c.codigo = l.combo and l.carrito = $1',
            [cartId]
        );
        res.json(productos.rows);
    } catch (err) { console.error(err.message) }
...
app.post('/enlace/prueba/', (req, res) => {
    const { mydata } = req.body;
    console.log(mydata);
})
...
USERS = [{
    ...
    "id":1032428147,
    "nickname":"TESTIU3HJ0JW",
    "password":"qatest9837",
    "site_status":"active",
    "email":"test_user_89592429@testuser.com"
},
{
    'rol': 'comprador',
    'access_token': 'APP_USR-6338833763403721-120403-8865b44e6df7227395b7fc146fbdb71d-1032431236',
    "id":1032431236,
    "nickname":"TESTYUIL0UD3",
    "password":"qatest3549",
    "site_status":"active",
    "email":"test_user_86004309@testuser.com"
}]
